...
"""
        ...
        response = await self.llm_client.generate_structured(
            prompt=prompt,
            system_prompt="You are an expert at identifying architecture decisions in conversations.",
            schema={"type": "json_object"},
        )

        # Handle potentially different structure from generate_structured depending on implementation
        # Assuming response is the parsed dictionary

        extracted_decisions = response.get("decisions", [])
        if not extracted_decisions and isinstance(response, dict):
            # Fallback if the response is nested or structure differs
             pass
